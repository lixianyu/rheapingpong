...
#else
static TimerHandle_t TxTimeoutTimer;
static TimerHandle_t RxTimeoutTimer;
static TimerHandle_t RxTimeoutSyncWord;
#endif
/*
...
void SX1276SetSleep( void )
{
    ...
    #else
    xTimerStop(RxTimeoutTimer, portMAX_DELAY);
    xTimerStop(TxTimeoutTimer, portMAX_DELAY);
    #endif
...
void SX1276SetRx( uint32_t timeout )
{
    ...
    if( timeout != 0 )
    {
        ...
        #else
        xTimerChangePeriod(RxTimeoutTimer, timeout / portTICK_PERIOD_MS, portMAX_DELAY);
        xTimerStart(RxTimeoutTimer, 100 / portTICK_PERIOD_MS);
        #endif
    }
    ...
    if( SX1276.Settings.Modem == MODEM_FSK )
    {
        ...
        if( rxContinuous == false )
        {
            ...
            #else
            xTimerChangePeriod(RxTimeoutSyncWord, SX1276.Settings.Fsk.RxSingleTimeout / portTICK_PERIOD_MS, portMAX_DELAY);
            xTimerStart(RxTimeoutSyncWord, 5000 / portTICK_PERIOD_MS);
            #endif
        }
    }
    else
...
void SX1276SetTx( uint32_t timeout )
{
    ...
    #else
    xTimerChangePeriod(TxTimeoutTimer, timeout / portTICK_PERIOD_MS, 5000 / portTICK_PERIOD_MS);
    #endif
    switch( SX1276.Settings.Modem )
    {
    case MODEM_FSK:
    ...
    #else
    xTimerStart(TxTimeoutTimer, 5000 / portTICK_PERIOD_MS);
    #endif
...
void SX1276SetTxContinuousWave( uint32_t freq, int8_t power, uint16_t time )
{
    ...
    TimerStart( &TxTimeoutTimer );
    #else
    xTimerStart(TxTimeoutTimer, 100 / portTICK_PERIOD_MS);
    #endif
    SX1276SetOpMode( RF_OPMODE_TRANSMITTER );
}
    ...
    case RF_RX_RUNNING:
        if( SX1276.Settings.Modem == MODEM_FSK )
        {
            ...
            if( SX1276.Settings.Fsk.RxContinuous == true )
            {
                ...
                //TimerStart( &RxTimeoutSyncWord );
                xTimerStart(RxTimeoutSyncWord, 100 / portTICK_PERIOD_MS);
            }
            else
            {
                SX1276.Settings.State = RF_IDLE;
                //TimerStop( &RxTimeoutSyncWord );
                xTimerStop(RxTimeoutSyncWord, portMAX_DELAY);
            }
        }
